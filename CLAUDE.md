...
- `POST /api/skills` never publishes directly — it always creates a review request. Public read endpoints under `src/app/api/skills/**` (search, detail, download, install, versions) require no auth for *access* — anyone can call them — though `GET /api/skills` now calls `auth()` to resolve the current viewer's own rating (`userRating`) when present, so it is not a purely anonymous code path anymore. The actual decision endpoints live under `src/app/api/review-requests/**`.
