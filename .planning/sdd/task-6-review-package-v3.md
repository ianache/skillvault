...
## Stat
 ...
 src/lib/review/service.test.ts              | 164 +++++++++++++++++++++++++++-
 src/lib/review/service.ts                   | 124 +++++++++++++++++++--
 src/lib/review/types.ts                     |   1 +
 9 files changed, 616 insertions(+), 98 deletions(-)
...
@@ -88,20 +140,21 @@ function createFakeClient(files: Array<Record<string, unknown>> = []): ReviewDat
       }
...
@@ -149,15 +202,118 @@ test("per-file comment accepts SKILL.md and an attached file", async () => {
     created_at: 1,
   };
...
index 7b49c6e..0498ca6 100644
--- a/src/lib/review/service.ts
+++ b/src/lib/review/service.ts
@@ -120,20 +120,112 @@ async function replaceFiles(id: number, files: ReturnType<typeof validateSubmiss
...
@@ -178,30 +270,31 @@ export async function updateReviewRequest(
   client: ReviewDatabaseClient
 ): Promise<ReviewRequest> {
...
+      Math.floor(Date.now() / 1000),
 ...
 export async function listReviewRequests(
   ...
   actor: ReviewActor,
@@ -215,21 +308,25 @@ export async function listReviewRequests(
   }
   if (query.status) {
     clauses.push("status = ?");
...
@@ -285,20 +382,27 @@ export async function decideReviewRequest(
   }
...
index 53967e9..a64a5e1 100644
--- a/src/lib/review/types.ts
+++ b/src/lib/review/types.ts
@@ -87,11 +87,12 @@ export type ReviewRequestSummary = Omit<ReviewRequest, "rawContent">;
