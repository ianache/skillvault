...
## Stat
 .superpowers/sdd/task-6-report.md           |  37 +++++++++
 ...
 src/lib/review/service.test.ts              |  84 ++++++++++++++++++++-
 src/lib/review/service.ts                   |  97 +++++++++++++++++++++++-
 7 files changed, 432 insertions(+), 81 deletions(-)

## Diff
diff --git a/.superpowers/sdd/task-6-report.md b/.superpowers/sdd/task-6-report.md
new file mode 100644
index 0000000..e3e0863
--- /dev/null
...
+function createFakeClient(files: Array<Record<string, unknown>> = []): FakeClient {
   ...
   const request = {
     ...
     version: "1.0.0",
     schema_version: "1.1",
@@ -54,33 +62,62 @@ function createFakeClient(files: Array<Record<string, unknown>> = []): ReviewDat
...
@@ -88,20 +125,21 @@ function createFakeClient(files: Array<Record<string, unknown>> = []): ReviewDat
       }
...
@@ -149,15 +187,53 @@ test("per-file comment accepts SKILL.md and an attached file", async () => {
     created_at: 1,
   };
...
index 7b49c6e..42412cf 100644
--- a/src/lib/review/service.ts
+++ b/src/lib/review/service.ts
@@ -120,20 +120,107 @@ async function replaceFiles(id: number, files: ReturnType<typeof validateSubmiss
...
@@ -215,21 +302,25 @@ export async function listReviewRequests(
   }
   if (query.status) {
     clauses.push("status = ?");
...
@@ -285,20 +376,22 @@ export async function decideReviewRequest(
   }
